...
function checkSession() {
    ...
    window.df.apis.user.getSession({"body": {}}, function(response) {
        ...
        } else 
        {
            ...
            document.getElementById("quemain").style.display = "block";
...
{
    selectval();
    if (myobj.curstatus == "accepted" || myobj.curstatus == "enroute" || myobj.curstatus == "origin") 
    {
        var drvindex = myobj.logd.length - 1;
        socket.emit("bumpupofeng", myobj["logd"][drvindex][3]);
        audit(myobj["logd"][drvindex][3], "reverted", 0);
    }
}
function drvque(controllerstatus) 
{
    controllerque = controllerstatus;
    var drvclrvar = controllerstatus.drvclr;
    var drvclrformat = "";
    if (drvclrvar.length > 0) {
        drvclrvar.forEach(function(entry) {
            var drvarrsp = entry.split('@');
            drvclrformat += "<td id='queborder'><div class='idv'>" + drvarrsp[0] + "<br>" + drvarrsp[2] + "<br></div>" + drvarrsp[1] + "</td>";
        });
    }
    var drvengvar = controllerstatus.drvengag;
    var drvengformat = "";
    if (drvengvar.length > 0) 
    {
        drvengvar.forEach(function(entry) {
            var drvarrsp = entry.split('@');
            drvengformat += "<td id='queborder'><div class='idv'>" + drvarrsp[0] + "<br>" + drvarrsp[2] + "<br></div>" + drvarrsp[1] + "</td>";
        });
    }
    document.getElementById('que').innerHTML = "<div class='lonwat'>Longest Waiting::</div><table id='tdrvclr'><tr>" + drvclrformat + "</tr></table>";
    document.getElementById('que').innerHTML += "<div class='engaged'>Engaged::</div><table id='tdrveng'><tr>" + drvengformat + "</tr></table>";
    if (drvclrvar.length > 0) 
    {
        document.getElementById("tdrvclr").style.display = "block";
    }
    if (drvengvar.length > 0) 
    {
        document.getElementById("tdrveng").style.display = "block";
    }
}
function canceljob() 
{
...
function audit(driveridval, statusval, flagval) 
{
    ...
    complete.push(currentdatetimestr);
    complete.push(statusval);
    complete.push(userid);
    complete.push(driveridval);
    var tempbuffer = [];
    if (myobj.logc) {
        tempbuffer.push(myobj.logc);
    }
    tempbuffer.push(complete);
    var item = {"record": [{"_id": bookingref,"logc": tempbuffer,"flag": flagval,"curstatus": statusval}]};
    df.apis.localmongo.updateRecords({"table_name": "scarscollection","body": item}, function(response) {
        getRecords();
    }, crudError);
    myobj = [];
